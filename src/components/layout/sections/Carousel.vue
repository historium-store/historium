<template>
  <div class="mx-6 my-12">
    <div class="relative slide">
      <div class="carousel-inner relative overflow-hidden rounded-lg border-[3px]">
        <div
          v-for="(img, i) in images"
          :id="`slide-${i}`"
          :key="i"
          :class="`${active === i ? 'active' : 'left-full'}`"
          class="carousel-item inset-0 relative w-full transform transition-all duration-500 ease-in-out"
        >
          <img v-if="isSmall" class="block w-full max-h-[1200px]" :src="img.small" alt="slides" />
          <img v-else class="block w-full max-h-[371px]" :src="img.big" alt="slides" />
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      windowWidth: window.innerWidth,
      images: [
        {
          small: './src/assets/banner-square.jpg',
          big: './src/assets/banner-wide.jpg'
        },
        {
          small: './src/assets/banner-square.jpg',
          big: './src/assets/banner-wide.jpg'
        }
      ],
      active: 0,
      timerCountdown: 6000,
      interval: null
    }
  },
  computed: {
    isSmall() {
      if (this.windowWidth < 640) return true
      else return false
    }
  },
  mounted() {
    this.interval = setInterval(this.slide, this.timerCountdown)
    window.addEventListener('resize', () => {
      this.windowWidth = window.innerWidth
    })
  },
  methods: {
    slide() {
      if (this.active < this.images.length - 1) {
        this.active++
      } else this.active = 0
    }
  }
}
</script>

<style scoped>
.left-full {
  left: -100%;
}

.carousel-item {
  float: left;
  position: relative;
  display: block;
  width: 100%;
  margin-right: -100%;
  backface-visibility: hidden;
}

.carousel-item.active {
  left: 0;
}
</style>
